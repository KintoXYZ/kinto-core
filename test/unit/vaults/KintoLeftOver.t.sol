...
contract KintoLeftOverTest is Test {
    ...
    function testAcceptAndClaim_HappyPath() public {
        ...
        vm.prank(OWNER);
        leftover.updateUserInfo(
            alice,
            KintoLeftOver.UserInfo({amount: 123_456 * 1e6, claimed: false})
        );
    ...
    function testUpdateUserInfo_OnlyOwner() public {
        KintoLeftOver.UserInfo memory info = KintoLeftOver.UserInfo({
            amount: 10_000 * 1e6,
            claimed: false
        });
    ...
    function testUpdateUserInfo_RejectsClaimedFlag() public {
      ...
      vm.stopPrank();
        ...
        KintoLeftOver.UserInfo memory bad = KintoLeftOver.UserInfo({
            amount: 90_000 * 1e6,
            claimed: false
        });
    ...
    function testUpdateUserInfo_SetsThenClaim() public {
        ...
        leftover.updateUserInfo(
            bob,
            KintoLeftOver.UserInfo({amount: 777_777 * 1e6, claimed: false})
        );
    ...
    function testSetUsersInfo_NormalMultiUserThenClaims() public {
        ...
        assertEq(aAmt, 50_000 * 1e6);  assertFalse(aClaimed);
        assertEq(bAmt, 75_500 * 1e6);  assertFalse(bClaimed);
        assertEq(cAmt, 90 * 1e6);      assertFalse(cClaimed);
