...
contract KintoInitialDeployScript is Create2Helper, ArtifactsReader {
    ...
    function run() public {
        ...
        // Wallet Factory impl
        address walletfImplAddr = computeAddress(
            0,
            abi.encodePacked(
                type(KintoWalletFactory).creationCode, abi.encode(KintoWallet(payable(address(_walletImpl))))
            )
        );
        ...
        if (isContract(walletfImplAddr)) {
            ...
            console.log("Already deployed Kinto Wallet Factory implementation at", address(walletfImplAddr));
        } else {
        ...
        address walletFactoryAddr =
            computeAddress(0, abi.encodePacked(type(UUPSProxy).creationCode, abi.encode(address(_walletFactoryI), "")));
        ...
        bytes memory creationCodePaymaster =
            abi.encodePacked(type(SponsorPaymaster).creationCode, abi.encode(address(_entryPoint)));
        ...
        address paymasterProxy = computeAddress(
            0, abi.encodePacked(type(UUPSProxy).creationCode, abi.encode(address(_sponsorPaymasterImpl), ""))
        );
        if (isContract(paymasterProxy)) {
            ...
            _sponsorPaymaster.initialize(address(msg.sender), IKintoAppRegistry(address(0)), _kintoID);
        }

        // KYC Viewer
        address kycViewerAddr =
            computeAddress(0, abi.encodePacked(type(KYCViewer).creationCode, abi.encode(address(_walletFactory))));
        ...
        address kycViewerProxyAddr =
            computeAddress(0, abi.encodePacked(type(UUPSProxy).creationCode, abi.encode(address(_kycViewerImpl), "")));
        ...
        vm.writeLine(
            _getAddressesFile(), string.concat('"KintoWallet-impl": "', vm.toString(address(_walletImpl)), '",')
        );
        vm.writeLine(
            _getAddressesFile(),
            string.concat('"KintoWalletFactory-impl": "', vm.toString(address(_walletFactoryI)), '",')
        );
        vm.writeLine(
            _getAddressesFile(), string.concat('"KintoWalletFactory": "', vm.toString(address(_walletFactory)), '",')
        );
        vm.writeLine(
            _getAddressesFile(), string.concat('"SponsorPaymaster": "', vm.toString(address(_sponsorPaymaster)), '",')
        );
        vm.writeLine(
            _getAddressesFile(),
            string.concat('"SponsorPaymaster-impl": "', vm.toString(address(_sponsorPaymasterImpl)), '",')
        );
        vm.writeLine(_getAddressesFile(), string.concat('"KYCViewer": "', vm.toString(address(_kycViewer)), '",'));
        vm.writeLine(
            _getAddressesFile(), string.concat('"KYCViewer-impl": "', vm.toString(address(_kycViewerImpl)), '"')
        );
