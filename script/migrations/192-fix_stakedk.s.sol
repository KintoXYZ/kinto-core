...
contract FixStakedKinto is MigrationHelper {
    ...
    function run() public override {
        ...
        _deployImplementationAndUpgrade("StakedKinto", "V11", bytecode);
        ...
        address[] memory users = new address[](records.length);
        uint256[] memory shares = new uint256[](records.length);
        uint256[] memory balances = new uint256[](records.length);
        for (uint256 i = 0; i < records.length; i++) {
            if (records[i].mint) {
                users[i] = records[i].user;
                shares[i] = records[i].shares;
            }
            balances[i] = stakedKinto.balanceOf(records[i].user);
        }
        _handleOps(
            abi.encodeWithSelector(StakedKinto.batchMintCurrentPeriodStake.selector, users, shares),
        ...
        _handleOps(
            abi.encodeWithSelector(StakedKinto.batchBurnCurrentPeriodStake.selector, users, shares),
            payable(_getChainDeployment("StakedKinto"))
        );

        require(stakedKinto.balanceOf(users[0]) == balances[0] - 61455800000000000000, "Did not burn");
        require(stakedKinto.balanceOf(users[1]) == balances[1] + 21666666649999900000, "Did not mint");
        require(
            stakedKinto.balanceOf(users[records.length - 1]) == balances[records.length - 1] + 3036400000000000000,
            "Did not burn"
        );
    }
}
